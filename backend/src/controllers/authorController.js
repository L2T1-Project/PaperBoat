...
class AuthorController {
    ...
    updateAuthor = async (req, res) => {
        try {
            ...
            const author = await this.authorModel.updateAuthor(Number(id), { name, orc_id });
    ...
    createPaperAuthor = async (req, res) => {
        try {
            ...
            const link = await this.authorModel.createPaperAuthor(Number(id), Number(paper_id), Number(position));
            ...
            if (error.code === '23505') {
                return res.status(409).json({ success: false, message: 'This author is already linked to that paper.' });
            }
            if (error.code === '23503') {
                return res.status(404).json({ success: false, message: 'Author or paper not found.' });
            }
    ...
    deletePaperAuthor = async (req, res) => {
        try {
            ...
            const link = await this.authorModel.deletePaperAuthor(Number(id), Number(paperId));

            if (!link) {
                return res.status(404).json({ success: false, message: 'Author-paper link not found.' });
            }
    ...
    lookupByOrcId = async (req, res) => {
        try {
            ...
            if (!orc_id) {
                return res.status(400).json({ success: false, message: 'orc_id query parameter is required.' });
            }

            const author = await this.authorModel.getAuthorByOrcId(orc_id);

            if (!author) {
                return res.status(404).json({ success: false, message: 'No author found with that ORC ID.' });
            }
