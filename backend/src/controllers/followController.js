...
class FollowController {
  ...
  followResearcher = async (req, res) => {
    try {
      ...
      const inserted = await this.followModel.followUser(
        followingUserId,
        followedUserId,
      );
      ...
      if (inserted) {
        ...
        this.followModel
          ...
          .catch((err) =>
            console.error("follow notification procedure error:", err),
          );
        return res
          .status(201)
          .json({ success: true, message: "Now following." });
      }
  ...
  unfollowResearcher = async (req, res) => {
    try {
      await this.followModel.unfollowUser(
        req.auth.userId,
  ...
  getFollowStatus = async (req, res) => {
    try {
      const following = await this.followModel.isFollowing(
        req.auth.userId,
        Number(req.params.userId),
      );
  ...
  getMyFollowers = async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "Failed to fetch followers." });
    }
  };
  ...
  getMyFollowing = async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "Failed to fetch following." });
    }
  };
}
